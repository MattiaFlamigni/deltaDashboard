...
@section("content")
    ...
    <form action="{{ route('quiz.update', $quiz) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="question" name="question" rows="3" required>{{ old('quiz', $quiz->question ?? '') }}</textarea>
        </div>
        ...
        @for ($i = 0; $i < 4; $i++)
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="answers_{{ $i }}" name="answers[]" value="{{ old('answers.' . $i, $quiz->answers[$i]->answer ?? '') }}" required>
            </div>
        @endfor
            ...
            <div>
                @for ($i = 0; $i < 4; $i++)
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="correct_answer" id="correct_answer_{{ $i }}" value="{{ $i }}" {{ $quiz->answers[$i]->correct ? 'checked' : '' }} required>
